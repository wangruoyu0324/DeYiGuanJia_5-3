...
public class CameraHelper implements Camera.PreviewCallback {
    ...
    private float previewSizeScale = 1.0f;
    ...
    private CameraHelper(Builder builder) {
        ...
        previewSizeScale = builder.previewSizeScale;
        ...
        synchronized (this) {
            try {
                ...
                if (cameraListener != null) {
                    cameraListener.onCameraOpened(mCamera, 0, displayOrientation, isMirror);
                }
            } catch (Exception e) {
                if (cameraListener != null) {
    ...
    private void setFixedFocus(Camera.Parameters cameraParameters){
    ...
    private int getCameraOri(int rotation) {
        ...
        if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT || isMirror) {
            if ((degrees + 180) % 180 == 0) {
                cameraRotation = degrees % 360;
            } else {
                cameraRotation = (degrees + 180)%360;
            }
        } else {
            cameraRotation = degrees % 360;
        }
//        cameraRotation = degrees;
    ...
    public void onPreviewFrame(byte[] nv21, Camera camera) {
        if (cameraListener != null) {
            cameraListener.onPreview(nv21, camera, cameraRotation);
        }
    }
...
//
//                //没有相机
//                if (specificCameraId == null || specificCameraId == -1) {
//                    if (cameraListener != null) {
...
//                }
                LogUtils.e("------------------mCameraId="+mCameraId);
...
//                parameters.setPreviewSize(previewSize.width, previewSize.height);
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
            if(previewSizeScale < 1.0f){
                float aspet = (float)width/(float)height;
                int scaleW = (int)((float)width * previewSizeScale);
                int scaleH = (int)((float)scaleW/aspet);
//                int scaleW = (int)(240);
//                int scaleH = (int)(320);
                previewDisplayView.setLayoutParams(new FrameLayout.LayoutParams(scaleW, scaleH));
            }
            start();
        }
    ...
    public static final class Builder {
        ...
        public Builder previewSizeScale(float val){
            previewSizeScale = val;
            return this;
        }
