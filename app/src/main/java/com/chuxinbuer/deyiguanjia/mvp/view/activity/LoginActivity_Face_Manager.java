...
public class LoginActivity_Face_Manager extends BaseActivity implements ViewTreeObserver.OnGlobalLayoutListener, IBaseView {
    ...
    private static final String TAG = LoginActivity_Face_Manager.class.toString();
    ...
    protected void init() {
        ...
        mRemainTime.setOnFinishTimeClick(new CountDownButton.OnFinishTimeClick() {
            ...
            public void onFinishTimeClick() {
                if (Common.isTopActivity(LoginActivity_Face_Manager.this, "com.chuxinbuer.deyiguanjia.mvp.view.activity.LoginActivity_Face")) {
                    Common.openActivity(LoginActivity_Face_Manager.this, LoginActivity.class);
                }
                stopTask();
            }
        });
        ...
        if (Common.empty(MyApplication.getInstance().getFaceList_Manager())) {
            try {
    ...
    private void initCamera() {
        ...
        CameraListener cameraListener = new CameraListener() {
            ...
            public void onPreview(final byte[] nv21, Camera camera, int cameraRotation) {
                try {
                    if (!initSuccess) {
                        Common.openActivity(LoginActivity_Face_Manager.this, AuthActivity.class);
                        stopTask();
                        return;
                    }
...
//                        freshDrawFps();
                        ...
                        if (maxSimiIndexPair.simi >= AppConfigManager.getInitedAppConfig().getFace_similar()) {
                            ...
                            Map<String, String> map = new HashMap<>();
                            map.put("token", token);
                            new HttpsPresenter(LoginActivity_Face_Manager.this, LoginActivity_Face_Manager.this).request(map, Constant.FACELOG, false);


                            Bundle b = new Bundle();
                            b.putBoolean("isManager", true);
                            Common.openActivity(LoginActivity_Face_Manager.this, ChooseRefuseKindActivity_Manager.class, b);
                            stopTask();
                        } else {
                            //Stranger logic
    ...
    public void showResult(String status, String pRows, String url) {
        if (status.equals(ExceptionEngine._SUCCESS)) {
            if (url.equals(Constant.FACELIST)) {
                ...
                if (!Common.empty(MyApplication.getInstance().getFaceList_Manager())) {
                    mAllList.addAll(JSON.parseArray(MyApplication.getInstance().getFaceList_Manager(), FaceModel.class));
                }
                MyApplication.getInstance().setFaceList_Manager(JSON.toJSONString(mAllList));
