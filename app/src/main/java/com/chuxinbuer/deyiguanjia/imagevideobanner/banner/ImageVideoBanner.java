...
//        if (!Common.empty(AppConfigManager.getInitedAppConfig().getBannerposition())) {
//            mViewPager.setCurrentItem(AppConfigManager.getInitedAppConfig().getBannerposition());
//        } else {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case UPTATE_VIEWPAGER:
                    if (msg.arg1 != 0) {
                        if (mList.size() > autoCurrIndex) {
                            if (mList.get(autoCurrIndex).getType() == 1 && msg.arg1 == 1) {
                                if (isFirstVideo) {
